...
if __name__ == "__main__":
    ...
    if(args.nchunks <= 32): 
        ...
        subprocess.run(f"taskset -c {cpulist} python multiproc_run_fromuris.py {cpulist} {cliuris}", shell=True, check=True)
    else:
        # Run on both NUMA domains, assigning each Python process to its own domain
        cpulists = ["0-31", "32-63"]
        cliuris = [" ".join(ntupleuris[:32]), " ".join(ntupleuris[32:])]
        commands = [f"taskset -c {cpulist} python multiproc_run_fromuris.py {cpulist} {uris}" for cpulist, uris in zip(cpulists, cliuris)]
