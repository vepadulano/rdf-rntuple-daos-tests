...
def analysis(ntuplename, ntuplepath, procid):
    ...
    if (procid < 8):
        subprocess.run(f"taskset -pc {procid+8} {pid}", shell=True, check=True)
    else:
        subprocess.run(f"taskset -pc {procid-8} {pid}", shell=True, check=True)
    ...
    with open(f"multiproc_lhcb_process{procid}.txt", "a+") as f:
...
if __name__ == "__main__":
    ...
    outcsv = f"multiproc_lhcb_100x_{args.storagetype}_{args.nchunks}cores.csv"

    with multiprocessing.Pool(processes=args.nchunks) as pool:
        for _ in range(10):
            watch = ROOT.TStopwatch()
