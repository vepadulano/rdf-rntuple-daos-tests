...
parser.add_argument("--coresperworker", help="Number of cores per worker", type=int)
...
if __name__ == "__main__":
    ...
    sconf = pyspark.SparkConf().setAll([("spark.master", "spark://hl-d101.head.cm.us.cray.com:7077"),
                                        ...
                                        ("spark.executor.instances", args.nworkers),
                                        ("spark.executor.cores", args.coresperworker),
                                        ("spark.cores.max", args.nworkers * args.coresperworker)])
    sc = pyspark.SparkContext(conf=sconf)
    initialize(myinit)
    df = Spark.MakeNTupleDataFrame(ntuplename, ntupleuris, sparkcontext=sc)

    outcsv = f"distrdf_lhcb_100x_delphi-013_{args.storagetype}_{args.nworkers * args.coresperworker}cores.csv"
