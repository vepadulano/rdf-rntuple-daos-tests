...
def create_ntuples():
    ...
    out_cpp_filename = f"gen_lhcb_{HOSTNAME}_{args.storagetype}_{args.nchunks}chunks.cpp"
...
def analysis(ntuplename, ntuplepath, procid):
    ...
    with open(f"multiproc_lhcb_{HOSTNAME}_process{procid}.txt", "a+") as f:
        f.write(f"{round(elapsed, 3)},{walltime},{readthroughput},{readpayload},{HOSTNAME},{procid}\n")


if __name__ == "__main__":
    ntuplename = "DecayTree"
    ntupleuris = create_ntuples()
    outcsv = f"multiproc_lhcb_50x_{HOSTNAME}_{args.storagetype}_{args.nchunks}cores.csv"

    with Pool(processes=args.nchunks) as pool:
        for _ in range(10):
            watch = ROOT.TStopwatch()
            pool.starmap(analysis, [(ntuplename, ntuplepath, procid) for procid, ntuplepath in enumerate(ntupleuris)])
            elapsed = watch.RealTime()
